...
- **Consistency panel** — same gauge instrument (`.gauge-card`/`.gauge-track`/`.gauge-fill`) reused for a different rule: best single day's share of total profit against the prop firm's consistency limit. Only rendered per-account when the rule profile actually sets a `consistencyPercent`, so accounts without the rule don't show an empty/meaningless gauge.
- **Analytics cards** (`.analytics-card`) — bordered card, mono uppercase title, Recharts bar chart. Win-rate charts (by tag/hour/playbook) use `--accent` for bars since the value is a ratio, not P&L. The R-multiple distribution chart is the one exception that colors by `--pnl-pos`/`--pnl-neg` per bucket — justified because R-multiple sign directly reflects the trade's real outcome, not decoration.
- **Playbook cards** (`.playbook-card`) — a lightweight card grid (`.playbooks-list`), name + optional criteria text, used for the setup library. Deleting a playbook only unlinks it from trades (doesn't touch trade history) — the confirm copy says so explicitly.
